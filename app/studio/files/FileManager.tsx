...
export const FILE_ENDPOINTS = {
  ...
  presign: "/api/studio/files/presign"
} as const;
...
async function uploadToStore(file: File, onFraction: (fraction: number) => void): Promise<UploadedObject> {
  if (file.size === 0) {
    throw new Error(
  ...
  const presigned = await post<PresignResponse>(FILE_ENDPOINTS.presign, {
    fileName: file.name,
    contentType,
    byteSize: file.size
  });
...
export function FileManager({ categories, categoriesTruncated, storageReady }: FileManagerProps) {
  ...
  const addFile = useCallback(
    async (chosen: File[]) => {
      ...
      try {
        const object = await uploadToStore(file, (fraction) =>
...
function FileDetailPanel({
  ...
  const replace = useCallback(
    async (chosen: File[]) => {
      ...
      try {
        const object = await uploadToStore(upload, (fraction) =>
