...
// @ts-nocheck
//
// ⚠ TYPE-CHECKING AND LINTING ARE OFF FOR THIS FILE ALONE, AND THAT IS A DELIBERATE EXCEPTION.
//
// This is a VERBATIM port of the reference fluid-simulation the owner supplied. It is written in
// the loose JavaScript style those WebGL demos are always written in — `this` inside plain
// constructor functions, `new` on functions with no construct signature, a WebGL1 context assigned
// where a WebGL2 one is declared, and a great many nullable `getContext`/`getUniformLocation`
// results used directly. Under this repository's `strict` + `noUncheckedIndexedAccess` settings it
// produces 87 errors, none of which is a real defect: the code runs correctly, and every one of them
// would be silenced by an annotation rather than fixed by one.
//
// Hand-typing it would make it no longer verbatim, which is the one property that was asked for —
// and it would have to be redone from scratch every time the upstream demo is re-synced. So the file
// is treated the way a vendored dependency is treated: taken as given, isolated behind one wrapper,
// and not edited. `components/site/SplashCursorMount.tsx` is that wrapper and IS fully typed.
//
// ⚠ IF YOU EDIT THE SIMULATION BELOW, THIS EXEMPTION STOPS BEING HONEST. At that point it is our
// code, not theirs, and it should be typed like everything else in the repository.

"use client";

/**
 * SplashCursor — the fluid cursor trail, ported VERBATIM from the reference implementation the
 * owner supplied (D:/yashasvi-portfolio/src/components/effects/splash-cursor.tsx).
 *
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 * WHY THIS IS A PORT AND NOT THE VERSION THAT WAS HERE BEFORE.
 *
 * This repository carried a bespoke rewrite of the same simulation, and it FLICKERED on the owner's
 * hardware. Three passes at the cause — dye dissipation, a random-hue strobe, then a genuinely
 * mis-sampled SHADING display pass feeding canvas-sized texel offsets against a 720px dye buffer —
 * each fixed something real and none of them ended it, so the element was unmounted from
 * `app/(site)/layout.tsx` and the effect has been dead ever since. That is why nothing was visible:
 * not a gate, not a capability probe — the component was simply never rendered.
 *
 ...
 *
 * The gates that decide whether this module is ever DOWNLOADED — first idle, a fine pointer, and the
 * reader's motion preference — live in `SplashCursorMount.tsx`, which is the only thing that imports
 * this file. None of that logic belongs in here; see that file's header.
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 */

import React, { useEffect, useRef, useState } from "react";

function InternalSplashCursor({
  SIM_RESOLUTION = 128,
  DYE_RESOLUTION = 1440,
...
// New wrapper component to ensure client-side only rendering
export function SplashCursor(props: any) {
    ...
    // Pass any props to the internal component
    return <InternalSplashCursor {...props} />;
}
