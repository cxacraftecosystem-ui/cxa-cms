 ...
 *
 * ⚠ NOTHING IS REQUIRED. A bucket left alone inherits from the next smaller one that was set, and the
 * smallest falls back to the photograph's own crop. Six controls that all had to be filled in would be
 * six controls nobody used, so the common case is to touch one.
 *
...
import { Crop, ImageIcon, RotateCcw } from "lucide-react";
...
import { cropFrameStyle, FULL_CROP } from "@/lib/media/crop";
import {
  SCREEN_BUCKETS,
  emptyScreenFraming,
...
export function ScreenFramingPanel({ label, help, mediaId, value, onChange }: ScreenFramingPanelProps) {
  ...
  return (
    <section aria-labelledby={headingId} className="rounded-md border border-line-200 bg-surface-50 p-3">
      <h3 id={headingId} className="flex items-center gap-1.5 font-display text-sm font-semibold text-ink-900">
        <Crop aria-hidden="true" className="h-3.5 w-3.5 shrink-0 text-ink-500" />
        {label}
      </h3>
      {help ? <HelpText className="mt-1">{help}</HelpText> : null}
